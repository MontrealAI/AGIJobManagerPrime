...
Prime also exposes keeper/bot-friendly autonomy views for deterministic progress checks:
- Settlement (`AGIJobManagerPrime`): `isFinalizable`, `isCheckpointFailed`, `isExpirable`, `nextActionForJob`, `getAutonomyStatus`.
- Discovery (`AGIJobDiscoveryPrime`): `isShortlistFinalizable`, `isWinnerFinalizable`, `isFallbackPromotable`, `nextActionForProcurement`, `getAutonomyStatus`.
