...
Prime also exposes keeper/bot-friendly autonomy views for deterministic progress checks in discovery (`AGIJobDiscoveryPrime`): `isShortlistFinalizable`, `isWinnerFinalizable` (both pause-aware, including settlement pause), `isFallbackPromotable`, `nextActionForProcurement`, and `getAutonomyStatus`.
