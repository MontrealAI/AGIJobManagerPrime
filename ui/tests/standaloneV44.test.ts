...
describe('standalone v44 artifact', () => {
  ...
  it('renders executive review memo sections with preview/root source and advanced disclosure', () => {
    ...
    expect(html).toContain('Ready — simulation + estimate passed');
  ...
  it('runs identity preflight simulation and decodes registrar custom errors', () => {
    const html = loadHtml();
    expect(html).toContain('async function runIdentityMethodPreflight(action, label)');
    expect(html).toContain("await method.call({from:userAccount, value:'0'})");
    expect(html).toContain("await method.estimateGas({from:userAccount, value:'0'})");
    expect(html).toContain('function decodeIdentityCustomError(err)');
    [
      'RootInactive','InvalidLabel','ParentNotWrapped','ParentNotLocked','RegistrarNotAuthorised','ParentExpired',
      'NameUnavailable','WrappedOwnerMismatch','WrappedFuseMismatch','WrappedExpiryMismatch','IdentityNotEligible','NonexistentToken'
    ].forEach((name)=>expect(html).toContain(name));
  });
