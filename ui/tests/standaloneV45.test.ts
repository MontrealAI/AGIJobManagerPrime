...
describe('standalone v45 artifact', () => {
  const file = path.resolve(__dirname, '../agijobmanager_genesis_job_mainnet_2026-03-05-v45.html');
  const loadHtml = () => fs.readFileSync(file, 'utf8');

  it('exists and is versioned as v45', () => {
    expect(fs.existsSync(file)).toBe(true);
    expect(loadHtml()).toContain('Prime Mainnet Console · v45');
  });
  ...
  it('parses preview/rootHealth deterministically and maps status-to-recommendation', () => {
    ...
    expect(html).toContain('function unwrapTupleOutput(raw, expectedKey)');
    expect(html).toContain('internalType":"struct FreeTrialSubdomainRegistrarIdentity.PreviewView"');
    expect(html).toContain('internalType":"struct FreeTrialSubdomainRegistrarIdentity.RootHealthView"');
  ...
  it('keeps label validation guidance aligned with live preview status', () => {
    ...
    expect(html).toContain('99999999, gggggggg, eliteagent03');
  });
  ...
  it('renders not-applicable and graceful tokenURI parsing outcomes', () => {
    ...
    expect(html).toContain('derived locally; not yet confirmed on-chain');
  });
