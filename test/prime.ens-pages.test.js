...
contract('AGIJobManagerPrime ENS push hooks', (accounts) => {
  ...
  it('keeps legacy handleHook targets configurable for rollback cutovers', async () => {
    const legacyPages = await AGIJobPages.new('ipfs://legacy/', 'https://legacy.example/jobs/', { from: owner });
    await legacyPages.setJobManager(manager.address, { from: owner });
    await manager.setEnsJobPages(legacyPages.address, { from: owner });

    assert.equal(await manager.ensJobPages(), legacyPages.address);
  });

  it('replays cancel-time revoke/lock hooks from retained ENS replay state', async () => {
    await ensJobPages.setRevertHook(4, true, { from: owner });
    await ensJobPages.setRevertHook(5, true, { from: owner });
    await manager.createJob('ipfs://spec', web3.utils.toWei('10'), 100, 'details', { from: employer });
    await manager.cancelJob(0, { from: employer });

    await ensJobPages.setRevertHook(4, false, { from: owner });
    await ensJobPages.setRevertHook(5, false, { from: owner });
    await manager.syncEnsForJob(0, 4, { from: owner });
    await manager.syncEnsForJob(0, 5, { from: owner });

    assert.equal((await ensJobPages.revokeCalls()).toString(), '1');
    assert.equal((await ensJobPages.lockCalls()).toString(), '1');
    assert.equal(await ensJobPages.lastEmployer(), employer);
  });

  it('supports owner repair syncs after best-effort hook failures', async () => {
  ...
  it('keeps non-versioned broken targets non-fatal and observable', async () => {
    ...
    const tx = await manager.createJob('ipfs://spec', web3.utils.toWei('10'), 100, 'details', { from: employer });
    const hook = tx.logs.find((log) => log.event === 'EnsHookCallResult');
    assert.equal(hook.args.selector, '0x1f76f7a2');
    assert.equal(hook.args.success, false);
  });
});
