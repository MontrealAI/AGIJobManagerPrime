...
(async function main() {
  ...
  const cancelledLogs = await getLogs(provider, PRIME, iface.getEvent('JobCancelled').topicHash);
  ...
  for (const log of [...completedLogs, ...refundedLogs, ...expiredLogs, ...cancelledLogs]) {
