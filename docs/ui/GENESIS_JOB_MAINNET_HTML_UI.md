# Standalone HTML UI: `agijobmanager_genesis_job_mainnet_2026-03-05-v41.html`

## At-a-glance navigation

- [Quick start (safe and practical)](#quick-start-safe-and-practical)
- [Embedded network and contract assumptions (v41)](#embedded-network-and-contract-assumptions-v41)
...
`ui/agijobmanager_genesis_job_mainnet_2026-03-05-v41.html` is a **standalone, versioned HTML interface artifact** for AGIJobManager mainnet operations and demonstrations.

Version/lifecycle posture:
- This file is a point-in-time artifact (`v41`) committed in-repo for reproducible browser-based operations and review.
...
Grounding note: this list is based on visible controls, embedded ABIs, and in-page handlers in the `v41` file.

## Grounded page sections and expected outcomes (v41)
...
Inference boundary: this table describes visible UX and method wiring present in `v41`; it does not redefine protocol-level permissions.
...
## Write-capable transaction methods exposed in v41
...
Secondary:
- Developers comparing standalone snapshots (`v13` ... `v41`) during UI iteration.
...
```text
http://127.0.0.1:8000/agijobmanager_genesis_job_mainnet_2026-03-05-v41.html
```
...
Use either:
1. Direct file open (`file://.../ui/agijobmanager_genesis_job_mainnet_2026-03-05-v41.html`), or
...
```text
http://localhost:8000/ui/agijobmanager_genesis_job_mainnet_2026-03-05-v41.html
```
...
2. Confirm filename/path ends with `agijobmanager_genesis_job_mainnet_2026-03-05-v41.html`.
...
Write-gate conditions in v41 are explicitly tied to:
1. wallet connection,
2. Ethereum mainnet (`chainId 1`), and
3. in-page terms acceptance checkbox.

## Embedded network and contract assumptions (v41)
...
Use this `v41` runbook when you intentionally need a single-file, versioned browser interface. Use the broader UI docs when you need roadmap/development/testing context.

## Relationship to other UI artifacts in `ui/`

- `v41` is the documented standalone artifact for this runbook.
...
- This file documents a **versioned standalone artifact**: `v41`.
