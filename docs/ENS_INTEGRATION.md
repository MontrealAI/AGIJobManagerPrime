...
| NFT tokenURI is completion URI instead of `ens://` | Expected on current Prime: manager does not wire `useEnsJobTokenURI`, or ENS URI is empty/failing on a legacy manager | Check Prime `ensJobPages` pointer and ENSJP `jobEnsURI` / `effectiveJobEnsURI`; do not assume manager-side ENS NFT routing exists | Keep completion-URI/IPFS NFT flow on current Prime; only use ENS URI routing on a separately verified manager release |
