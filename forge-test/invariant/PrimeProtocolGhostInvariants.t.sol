...
contract PrimeGhostHandler is Test {
    ...
    function discoveryAction(uint256 procurementSeed, uint256 actorSeed, uint8 score, uint8 mode) external {
        ...
        if (branch <= 4) {
            ...
            bytes32 salt = keccak256(abi.encodePacked(pid, agent));
            vm.prank(agent);
            if (branch == 0) {
            ...
            address validator = actors[bound(actorSeed, 0, 3) * 3 + 2];
            address[] memory finalists = discovery.procurementFinalists(pid);
            address finalist = finalists.length == 0 ? agents[0] : finalists[bound(score, 0, finalists.length - 1)];
            bytes32 salt = keccak256(abi.encodePacked(pid, finalist, validator));
            vm.prank(validator);
            if (branch == 5) {
                try discovery.commitFinalistScore(
                    pid,
                    finalist,
                    keccak256(abi.encodePacked(pid, finalist, validator, uint8(bound(score, 0, 100)), salt)),
                    "",
                    new bytes32[](0)
                ) {}
                    catch {}
                ...
                try discovery.revealFinalistScore(
                    pid, finalist, uint8(bound(score, 0, 100)), salt, "", new bytes32[](0)
                ) {}
                    catch {}
            }
        } else {
