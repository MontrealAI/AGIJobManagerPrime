...
- `NETWORK` (optional helper for `npm run verify:prime`, default `sepolia`)
...
```bash
cd hardhat
NETWORK=mainnet npm run verify:prime
```
...
`verify:prime` exits non-zero if any target is not verified/already-verified, so CI/ops pipelines fail fast on partial verification.
